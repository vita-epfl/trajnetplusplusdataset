...
def generate_twoped_crossing_angle(num_ped=2, sim=None, radius=4, mode=None):
    positions = []
    goals = []
    speed = []
    agent_list = []
...
def generate_orca_trajectory(sim_scene, num_ped, min_dist=3, react_time=1.5, end_range=1.0, mode=None):
    ...
    if sim_scene == 'circle_crossing':
    ...
    elif sim_scene in {'two_ped', 'two_ped_angle'}:
        fps = 100
        sampling_rate = fps / 2.5
        ...
        if sim_scene == 'two_ped':
            ...
            trajectories, _, goals, speed = generate_twoped_crossing_angle(2, sim, mode=mode)
    else:
...
def main():
    ...
    parser.add_argument('--simulation_scene', default='circle_crossing',
                        choices=('circle_crossing', 'two_ped', 'two_ped_angle'))
